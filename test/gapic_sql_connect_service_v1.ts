...
describe('v1.SqlConnectServiceClient', () => {
  it('has servicePath', () => {
    const servicePath =
      sqlconnectserviceModule.v1.SqlConnectServiceClient.servicePath;
    assert(servicePath);
  });

  it('has apiEndpoint', () => {
    const apiEndpoint =
      sqlconnectserviceModule.v1.SqlConnectServiceClient.apiEndpoint;
    assert(apiEndpoint);
  });
  ...
  describe('getConnectSettings', () => {
    it('invokes getConnectSettings without error', async () => {
      ...
      client.innerApiCalls.getConnectSettings =
        stubSimpleCall(expectedResponse);
      const [response] = await client.getConnectSettings(request);
      assert.deepStrictEqual(response, expectedResponse);
      assert(
        (client.innerApiCalls.getConnectSettings as SinonStub)
          .getCall(0)
  ...
  describe('generateEphemeralCert', () => {
    it('invokes generateEphemeralCert without error', async () => {
      ...
      client.innerApiCalls.generateEphemeralCert =
        stubSimpleCall(expectedResponse);
      const [response] = await client.generateEphemeralCert(request);
      assert.deepStrictEqual(response, expectedResponse);
      assert(
        (client.innerApiCalls.generateEphemeralCert as SinonStub)
          .getCall(0)
          .calledWith(request, expectedOptions, undefined)
      );
    });
    ...
    it('invokes generateEphemeralCert without error using callback', async () => {
      ...
      const promise = new Promise((resolve, reject) => {
        client.generateEphemeralCert(
          ...
          (
            err?: Error | null,
            result?: protos.google.cloud.sql.v1.IGenerateEphemeralCertResponse | null
          ) => {
            if (err) {
              reject(err);
            } else {
